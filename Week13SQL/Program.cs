...
{
    SQLiteConnection connection = new SQLiteConnection("Data Source=mydb.db; Verison = 3; New = True; Compress = True;");
    try
    {
        connection.Open();
...
{
    Console.Clear(); //konsooli puhastamine
    SQLiteDataReader reader; //andmebaasist lugemine
    SQLiteCommand command; //päringu käsk

    command = myConnection.CreateCommand(); //ühenduse loomine
    command.CommandText = "SELECT rowid, * FROM customer";
    
    reader = command.ExecuteReader(); // andmete lugemine ja salvestamine readerisse
...
static void InsertCustomer(SQLiteConnection myConnection)
{
    ...
    int rowInserted = command.ExecuteNonQuery(); //käsk mis lisab andmed tabelisse
...
static void FindCustomer (SQLiteConnection myConnection)
{
    ...
    {
        string readerRowId = reader["rowid"].ToString();
        string readerStringStringName = reader.GetString(1);
        string readerStringLastName = reader.GetString(2);
        string readerStringStatus = reader.GetString(3);

      Console.WriteLine($"Search result: ID: {readerRowId}. {readerStringStringName}, {readerStringLastName}. Status: {readerStringStatus} ");
    }
